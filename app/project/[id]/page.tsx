...
const [deleteConfirm, setDeleteConfirm] = useState(false);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
  setProject(data);
  setNotes(data.notes && data.notes !== "null" ? data.notes : "");
  setXLink(data.x_link && data.x_link !== "null" ? data.x_link : "");
  setDiscordLink(data.discord_link && data.discord_link !== "null" ? data.discord_link : "");
}
      setLoading(false);
    };
    load();
...
const handleSaveLinks = async () => {
  if (!project) return;
  setSavingLinks(true);
  const { error } = await supabase.from("projects").update({ x_link: xLink, discord_link: discordLink }).eq("id", project.id);
  if (error) console.error("Links error:", error.message);
  else setProject((p) => p ? { ...p, x_link: xLink, discord_link: discordLink } : p);
    ...
    <div className="relative min-h-screen bg-zinc-950">
      ...
      <div className="relative mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-3.5">
            <span className="text-sm text-zinc-500">Days Remaining</span>
            <span className="text-sm font-semibold text-emerald-400">{daysRemaining} days</span>
          </div>
          {project.wallets && (
            <div className="flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-3.5">
  ...
  <div className="space-y-3">
    ...
    <div className="flex gap-3">
      ...
      {xLink && (
        <a href={xLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-2 text-sm font-medium text-zinc-300 hover:text-white transition-colors">
      ...
      {discordLink && (
        <a href={discordLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-2 text-sm font-medium text-zinc-300 hover:text-white transition-colors">
